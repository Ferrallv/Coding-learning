...
Implementing timeouts in Go is easy and elefant thanks
...
/*
For example, suppose we're executing an external call that
returns its result on a channel c1 after two seconds.
...
func main() {
	...
	`After` awaits a value to be sent after the timeout of 1sec
	...
	*/
	...
	// If we allow for a longer timeout of 3sec, then
	// the receive from `c2` will succeed and we'll print
	// the result.
