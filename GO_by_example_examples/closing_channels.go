...
sent on it. This can be userful to communicate completion 
...
goroutine. When we have no more jobs for the worker we'll 
...
func main() {
	...
	`jobs` with j, more:= <-jobs. in this special 2-val form
	of receive, the `more` val will be `false` if `jobs` has been
	`closed` and all vals in the channel have already been 
